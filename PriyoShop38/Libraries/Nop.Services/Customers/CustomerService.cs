...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        public string GetCustomerPhoneById(int customerId)
        {
            ...
            if (customerInfo.Addresses != null && customerInfo.Addresses.Count > 0)
            { 
                phoneNumber = customerInfo.Addresses.FirstOrDefault()?.PhoneNumber;
            }
            return phoneNumber;
        }
        ...
        public Customer GetCustomerByReferCode(string referCode)
        {
            var customerObj = _customerRepository.Table.FirstOrDefault(c => c.NewReferCode == referCode);
            if (customerObj == null)
            {
        ...
        public CustomerCommission InsertCustomerCommissionInfo(string sql)
        {
            var result = _dbContext.SqlQuery<CustomerCommission>(sql, true).ToList();
            if (result.Any())
            {
                return result.FirstOrDefault();
            }
            return new CustomerCommission();
        }
